...
var districts = [].concat.apply([], Object.values(Districts));
 ...
 $("#district").autocomplete({
                source: districts,
                minLength: 4 // Minimum characters before showing suggestions
            });
          });
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
        ...
        if (isset($_POST['name']) && isset($_POST['state']) && isset($_POST['district']) && isset($_POST['phoneno']) && isset($_POST['password']) && isset($_POST['confirmpassword'])) {
            ...
            if (!preg_match($nameregex, $name)) {
                $errmsg = "*Name should be in the correct format";
            } elseif ($password != $confirmpassword) {
                $errmsg = "*Password and confirm password are not the same";
            } elseif (!preg_match($passwordregex, $password)) {
                $errmsg = "*Password must contain at least eight characters, including one uppercase letter, one lowercase letter, one number, and one special character";
            } else {
                $sql = "INSERT INTO `users` (`name`,`state`,`district`,`phoneno`,`password`,`utype`) VALUES ('$name','$state','$district','$phoneno','$password','f')";
                $query = mysqli_query($conn, $sql);
                if ($query) {
                    // Wait for 300ms and then redirect to userdetails.php
                    echo '<script>
                            setTimeout(function() {
                                window.location.href = "userdetails.php";
                            }, 300);
                          </script>';
                    exit(); // Important: Ensure that the script exits after redirection
                } else {
            ...
            <form action="adduser.php" method="post">
                <div class="user-details">
                    <div class="input-box">
                        ...
                        <input name="name" id="name" type="text" placeholder="Enter your name" value="<?php echo "$name"; ?>" required pattern="[a-zA-Z\s]*">
                    </div>
                    <div class="input-box">
                        <span class="details">State</span>
                        <input type="text" placeholder="Enter your state" name="state" id="state" value="<?php echo "$state"; ?>" required>
                    </div>
                    <div class="input-box">
                        <span class="details">District</span>
                        <input type="text" placeholder="Enter your district" name="district" id="district" value="<?php echo "$district"; ?>" required>
                    </div>
                    <div class="input-box">
                        <span class="details">Phone Number</span>
                        <input type="text" placeholder="Enter your phone number" name="phoneno" id="phoneno" value="<?php echo "$phoneno"; ?>" required>
                    </div>
                    <div class="input-box">
